...
with col1:
    ...
    job_description = st.text_area("JD", height=260, placeholder="Paste the full job posting here...", label_visibility="collapsed")

with col2:
    st.markdown("#### 👤 Your Background")
    input_method = st.radio("Input method", ["✍️ Type / Paste", "📄 Upload Resume PDF"], horizontal=True, label_visibility="collapsed")
    user_background = ""
    if input_method == "✍️ Type / Paste":
        user_background = st.text_area("Background", height=210, placeholder="Describe your experience, education, skills, and projects...", label_visibility="collapsed")
    else:
        ...
        if uploaded_file:
            try:
                ...
                chunks = re.findall(r'[A-Za-z0-9\s\.,\-\@\(\)\+\:\/]{20,}', content)
                text = ' '.join(chunks[:200])
                if len(text.strip()) > 100:
                    user_background = text[:4000]
                    st.success(f"✅ Resume extracted ({len(user_background)} chars)")
                    with st.expander("Preview extracted text"):
                        st.text(user_background[:500] + "...")
                else:
                    st.warning("Could not extract text. Please paste manually.")
                    user_background = st.text_area("Background (manual)", height=150, label_visibility="collapsed")
            except Exception as e:
                st.error(f"PDF error. Please paste manually.")
                user_background = st.text_area("Background (manual)", height=150, label_visibility="collapsed")
    ...
    if not job_description.strip() or not user_background.strip():
        ...
        if "error" in result:
            ...
            if match_data and "overall_score" in match_data:
                ...
                if requirements:
                    ...
                    for req in requirements:
                        ...
                        label = {"strong": "✅ Strong Match", "partial": "⚡ Partial Match", "weak": "⚠️ Weak Match", "missing": "❌ Missing"}.get(level, level)
                        score_dots = "●" * req.get("score", 0) + "○" * (5 - req.get("score", 0))
                        ...
                        <div class="match-card {card_class}">
                            ...
                            <div style="font-size:1rem; color:{score_color}; letter-spacing:2px">{score_dots}</div>
                        </div>
                        """, unsafe_allow_html=True)
            ...
            with tabs[0]:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    st.download_button("⬇️ Download Cover Letter", data=cover, file_name="cover_letter.txt", mime="text/plain", use_container_width=True)
                if bullets:
                    st.markdown("---")
                    st.markdown("**Resume Bullets**")
                    st.markdown(bullets)
                    with c2:
                        st.download_button("⬇️ Download Resume Bullets", data=bullets, file_name="resume_bullets.txt", mime="text/plain", use_container_width=True)
                st.download_button("⬇️ Download Full Application", data=final, file_name="full_application.txt", mime="text/plain", use_container_width=True)
            ...
            with tabs[2]:
                ...
                st.download_button("⬇️ Download Email", data=email_content, file_name="followup_email.txt", mime="text/plain", use_container_width=True)
